...
def test_simple_workerprocess(ui_updater):
    ...
    try:
        ...
        ui_updater.add_text_to_output(
            f"Loader has _tempfiles: {hasattr(loader, '_tempfiles')}\n"
        )
        ...
        ui_updater.add_text_to_output(
            f"  Name: {worker.name if hasattr(worker, 'name') else 'N/A'}\n"
        )
        ...
        if worker.is_alive():
            ...
            ui_updater.add_text_to_output(
                f"WorkerProcess finished with exitcode: {worker.exitcode}\n"
            )
