...
def test_ssh_connection(ui_updater):
    ...
    app_dir = os.path.dirname(briefcase_ansible_test.__file__)
    ...
    ui_updater.add_text_to_output(f"Found {len(night2_hosts)} hosts in 'night2' group\n")
    
    # Use the first host from night2 group
    target_host = night2_hosts[0]
    ui_updater.add_text_to_output(f"✅ Selected host from night2 group: {target_host.name}\n")
    ...
    if host_vars:
        ...
        for key, value in host_vars.items():
            if not key.startswith('ansible_') or key in ['ansible_host', 'ansible_user', 'ansible_port']:
    ...
    if os.path.exists(key_path):
        ...
        if os.path.exists(pub_key_path):
            ...
            ui_updater.add_text_to_output(f"📋 Ed25519 Public key:\n{pub_key_content}\n")
            ...
            if pub_key_content.startswith('ssh-ed25519'):
                ...
                ui_updater.add_text_to_output("⚠️ Key does not appear to be Ed25519 format\n")
                ui_updater.add_text_to_output("   Expected to start with 'ssh-ed25519'\n")
    ...
    # Try to connect
    ui_updater.add_text_to_output(f"🚀 Attempting SSH connection to {target_host.name}...\n")
