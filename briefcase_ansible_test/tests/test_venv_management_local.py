...
Tests the same 4 core scenarios as test_venv_executor.py but with real execution instead of mocks.
"""
...
""")
            ...
            (inventory_dir / 'sample_inventory.ini').write_text("[localhost]\nlocalhost ansible_connection=local")
    ...
    def test_temporary_venv_no_name(self, temp_dir):
        ...
        import subprocess
        ansible_check = subprocess.run(['which', 'ansible-playbook'], capture_output=True, text=True)
        ...
        assert result.venv_name.startswith("temp_"), f"Expected temp_ venv name, got {result.venv_name}"
    ...
    def test_persistent_venv_with_existing(self, temp_dir):
        ...
        assert any("Using existing venv" in msg for msg in result.messages), f"Expected existing venv message in {result.messages}"
...
""")
        ...
        assert not any("Metadata saved" in msg for msg in result.messages), f"Unexpected metadata save message in {result.messages}"
        
        print("\nFailed execution test completed successfully!")
    
    def test_all_parameters_flow_through(self, temp_dir):
        """Test that all parameters flow through the function chain correctly using real Ansible.
        ...
        assert any("Metadata saved" in msg for msg in result.messages), f"Expected success message in {result.messages}"
